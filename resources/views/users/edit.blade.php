...
@section('content')
    ...
    <form action="{{ route('user.update', $user->id) }}" method="POST" class="row gy-4">
        @method('PUT')
        @csrf
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            ...
            <input name="password" type="text" placeholder="Empty to keep the current password" style="font-size: 11px; text-align: left; padding: 10px;" class="form-control" />
            <div class="form-text text-danger">
                @error('password')
                    {{ $message }}
                @enderror
